...
@REM echo关闭
@echo off
@REM Windows批处理处理机制是按行读取，每行执行前先做预处理
@REM（类似%var%都会被扩展到实际值）
@REM 变量延迟扩展，延迟变量引用方法 !var!
...
@REM "x!architecture!" 变量引用!var!, 变量为空，则只剩下"x"
@REM 用意 ？？？
if "x!architecture!" == "x" (
...
@REM 设置静态/动态编译，debug/release版本
	call::solution_compilation_flag_calculation
@REM 获取代码生成器的版本号，v3.0及以上支持flatdata
...
@REM 添加头文件/源文件
		...
		for %%i in (%custom_type_folder%\*) do (
@REM -I + idl_location ， %%i 在最后面。 这里应该存在错误 ？？？
...
@REM RTI_LANGUAGE_CPP_TRADITIONAL
